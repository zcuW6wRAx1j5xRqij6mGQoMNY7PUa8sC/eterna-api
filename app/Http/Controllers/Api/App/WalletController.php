...
class WalletController extends ApiController
{
    ...
    {
        $isOtc = $request->query('is_otc', 0);

        return $this->ok($spotWallet($request->user(), $isOtc));
    }
    ...
    public function transferList(Request $request)
    {
        ...
        $data = UserWalletSpotFlow::where('uid', $request->user()->id)
            ->whereIn('flow_type', [SpotWalletFlowEnums::FlowTypeTransferIn, SpotWalletFlowEnums::FlowTypeTransferOut])
