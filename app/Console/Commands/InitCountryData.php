...
class InitCountryData extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:init-country-data';
    ...
    public function handle()
    {
        ...
        collect($data)->chunk(100)->each(function($items){
            ...
            $items->each(function($item) use(&$inserts){
                $inserts[] = [
                    'name'=>$item['en'],
                    'phone_code'=>str_replace('+','00', $item['code']),
                    'status'=>1,
            ...
            if ($inserts) {
                PlatformCountry::insert($inserts);
            }
        });
